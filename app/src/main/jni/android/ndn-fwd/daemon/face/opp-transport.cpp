...
void OppTransport::sendNextPacket() {
    if(!m_intrQueue.empty())
        performSend(this->getFace()->getId(), m_intrQueue.front().packet);
    else if(!m_dataQueue.empty())
        performSend(this->getFace()->getId(), m_dataQueue.front().packet);
        else
...
void OppTransport::doSend(Packet&& packet) {
    ...
    if(currently == TransportState::UP && m_intrQueue.size() == 1) {
